    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-24">
        <div className="mb-12 border-b border-slate-200 dark:border-slate-800 pb-6">
          <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Welcome back{currentUser ? `, ${currentUser.full_name}` : ''}</h1>
          <p className="text-slate-500 mt-2">{currentUser ? `${currentUser.role} account • ${currentUser.email}` : 'Verifying your saved account…'}</p>
          {currentUser && <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-800"><ShieldCheck className="w-4 h-4" /> Profile loaded from PostgreSQL</div>}
          {profileError && <p role="alert" className="mt-3 text-sm text-rose-600">{profileError}</p>}
        </div>
