...
export default function JoinTeamModal({ isOpen, onClose, project }) {
  ...
  useEffect(() => {
    ...
    const loadCompatibilityScore = async () => {
      setStep(1);
      setSubmitted(false);
      ...
      try {
        ...
        setCompatibilityScore(result.compatibility_score);
      } catch (error) {
        const overlap = (project.skills || []).filter((skill) =>
          ['react', 'python', 'typescript', 'tailwind', 'fastapi', 'machine learning'].includes(String(skill).toLowerCase())
        ).length;
        const fallbackScore = project.id === 'smart-agri' ? 55 : Math.min(98, 70 + overlap * 8);
        setCompatibilityScore(fallbackScore);
      } finally {
        setIsCalculatingScore(false);
      }
    };

    loadCompatibilityScore();
  }, [isOpen, project]);
        ...
        <div className="flex justify-between items-end">
          <div>
            ...
            <div className={`text-4xl font-extrabold ${compatibilityScore >= 70 ? 'text-emerald-500' : 'text-amber-500'}`}>
              {isCalculatingScore ? '…' : `${compatibilityScore}%`}
            </div>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
              {isCalculatingScore ? 'Analyzing your fit…' : 'Live AI compatibility'}
            </p>
          </div>
