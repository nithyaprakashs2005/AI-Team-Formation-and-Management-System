...
export default function AiRecommendationPreview({ openAuthModal }) {
  ...
  const [liveRecommendation, setLiveRecommendation] = useState('Analyzing your fit…');
  ...
  useEffect(() => {
    ...
    const runMatch = async () => {
      try {
        const result = await matchApi.calculate({
          user_skills: ['React', 'Python', 'TypeScript', 'Tailwind', 'FastAPI'],
          target_skills: ['React', 'Python', 'Machine Learning'],
          timezone_difference_hours: 2,
          experience_level_match: 0.9,
        });

        if (isMounted) {
          setLiveScore(result.compatibility_score);
          setLiveRecommendation(result.recommendation_level);
        }
      } catch (error) {
        if (isMounted) {
          setLiveScore(84);
          setLiveRecommendation('High Match');
        }
      }
    };
    ...
    <section id="ai-preview" className="relative py-24 bg-slate-50 dark:bg-slate-950 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-sm font-semibold text-indigo-700 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-300">
            <Sparkles className="h-4 w-4" />
            <span>Live score: {liveScore}% • {liveRecommendation}</span>
          </div>
        </div>
